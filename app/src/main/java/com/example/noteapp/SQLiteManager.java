...
public class SQLiteManager extends SQLiteOpenHelper
{
    ...
    private static final String PDL_FIELD = "PDL";
    private static final String FDL_FIELD = "FDL";
...
public void addNoteToDatabase(Note note)
{
    ...
    contentValues.put(PDL_FIELD, note.getPDL());
    contentValues.put(FDL_FIELD, note.getFDL());
   ...
   try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
   {
       if(result.getCount() != 0)
       {
           while (result.moveToNext())
           {
               ...
               String PDL = result.getString(4);
               String FDL = result.getString(5);
               String stringDeleted = result.getString(6);
               Date deleted = getDateFromString(stringDeleted);
               Note note = new Note(id,title,desc,PDL,FDL,deleted);
